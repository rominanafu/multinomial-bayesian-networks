...
df$duracion <- ifelse(df$end >= df$start,
                      df$end - df$start,
                      (24*60 - df$start) + df$end)

# Función para calcular translape de minutos con un intervalo
overlap_minutes <- function(start, end, interval_start, interval_end) {
  # Ajustamos para viajes que cruzan medianoche
  if (end < start) end <- end + 24*60
  start <- start %% (24*60)
  end <- end %% (24*60)
  
  pmax(0, pmin(end, interval_end) - pmax(start, interval_start))
}
...
# Hora pico sí/no
df$horaPico <- factor(ifelse(df$porcentaje_pico > 0.5, "Pico", "No Pico"))
